...
import { ArrowRight, Play, Shield, Star, Users, ChevronDown } from "lucide-react";
...
export default function Hero() {
  ...
  // ── N'activer les bulles/gouttes que sur écrans larges (evite le coût sur mobile) ──
  useEffect(() => {
    if (typeof window === "undefined") return;
    const check = () => {
      setEnableBackgroundAnim(window.innerWidth >= 768);
    };
    check();
  ...
  /* ── Données bulles & gouttes stables ── */
  ...
  return (
    <section
      id="hero"
      aria-label="Makclean — accueil et présentation"
      style={{
        position: "relative",
        minHeight: "100vh",
        display: "flex",
    ...
    >
      {/* Texte d'introduction pour lecteurs d'écran (évite de répéter la meta description) */}
      <p className="sr-only">
        Makclean, expert en nettoyage professionnel en Hainaut : devis sous 24 h, équipe certifiée et résultats garantis.
        Retrouvez le détail des prestations et des zones sur le site.
      </p>
      ...
      <div aria-hidden="true" style={{ position: "absolute", inset: 0, zIndex: 0, overflow: "hidden" }}>
        ...
        >
          <path d="M0,60 C240,100 480,20 720,60 C960,100 1200,20 1440,60 L1440,120 L0,120 Z" fill="rgba(59,130,246,0.15)" />
          <path d="M0,80 C360,40 720,100 1080,60 C1260,40 1380,70 1440,80 L1440,120 L0,120 Z" fill="rgba(96,165,250,0.1)" />
        </svg>
      </div>

      {/* ── Bulles / gouttes (désactivées sur mobile pour les perfs) ── */}
      ...
      >
        ...
        >
          La propreté,{" "}
          <span style={{
            background: "linear-gradient(135deg, #1d4ed8, #0ea5e9, #38bdf8)",
        ...
        >
          ...
          <span className="sr-only">Types de missions : résidentiel, commercial, industriel, après chantier, particuliers</span>
          <span aria-hidden="true" style={{ color: "#2563eb", fontSize: "18px", fontWeight: 700, minWidth: "200px", textAlign: "left" }}>
            {displayText}
            <span style={{
        ...
        {/* Sous-titre */}
        <motion.p
          style={{ fontSize: "clamp(15px, 2vw, 18px)", color: "#475569", maxWidth: "580px", lineHeight: 1.7, margin: 0 }}
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          {[
            { icon: <Shield aria-hidden="true" style={{ width: "14px", height: "14px", color: "#2563eb" }} />, text: "Assurance RC Pro" },
            { icon: <Star aria-hidden="true" style={{ width: "14px", height: "14px", fill: "#f59e0b", color: "#f59e0b" }} />, text: "98 % de satisfaction" },
            { icon: <Users aria-hidden="true" style={{ width: "14px", height: "14px", color: "#2563eb" }} />, text: "Intervention rapide" },
          ].map((badge, i) => (
            <div key={i} style={{
    ...
    Voir nos réalisations
        ...
        <motion.div
          style={{
            ...
            gap: "1px", width: "100%", maxWidth: "700px", marginTop: "16px",
...
{STATS.map((stat, i) => {
  ...
  return (
    <div key={i} style={{
      ...
      <span style={{ fontSize: "clamp(20px, 3vw, 28px)", fontWeight: 800, color: "#1d4ed8", lineHeight: 1 }}>
        <CountUp end={num} suffix={suffix} duration={2000} />
      </span>
      <span style={{ fontSize: "11px", color: "#64748b", fontWeight: 500, textAlign: "center" }}>
        {stat.label}
      </span>
    </div>
  );
      ...
      >
        <span style={{ fontSize: "10px", color: "#94a3b8", fontWeight: 500, letterSpacing: "2px", textTransform: "uppercase" }}>
          Découvrir
        </span>
